...
export default function ERPLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] text-[#1E293B] font-sans antialiased flex flex-col md:flex-row w-full relative">
      ...
      <nav className="md:hidden fixed bottom-2 left-3 right-3 bg-white/90 backdrop-blur-md border border-slate-200/80 rounded-2xl px-3 py-2.5 flex justify-around items-center z-50 shadow-[0_8px_32px_rgba(0,0,0,0.08)]">
        {sidebarItems.map((item) => {
          const isActive = pathname === item.route;
          return (
            <button
